...
function createCubeBuffer() {
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    new Float32Array(getCubeNormals()),
    gl.STATIC_DRAW
  );
  ...
  gl.bufferData(
    ...
    new Uint16Array(getCubeIndices()),
    gl.STATIC_DRAW
  );
}
...
function createStickerBuffer() {
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    new Float32Array(getStickerVerts()),
    gl.STATIC_DRAW
  );
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    new Float32Array(getStickerNormals()),
    gl.STATIC_DRAW
  );
  ...
  gl.bufferData(
    gl.ELEMENT_ARRAY_BUFFER,
    new Uint16Array(getStickerIndices()),
    gl.STATIC_DRAW
  );
}
...
function randomMove() {
  console.log(state.rubiksCube.hasWon());
  /*
  if (!state.isRotating) {
...
function setMove() {
  if (!state.isRotating) {
    cubeToRotateCoordinates = [-1, -1, -1];

    axisToRotate = Math.floor(Math.random() * 3);

    state.rubiksCube.cycles = 1;
    state.rubiksCube.setRotation(cubeToRotateCoordinates, axisToRotate, false);
  }
}
