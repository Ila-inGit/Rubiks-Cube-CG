...
var worldMatrix2 = utils.MakeTranslateMatrix(3, 0, 0);
...
function drawScene() {
  ...
  if (isSecond) {
    if (deltaRotationAngles[0] != 0) {
      let rotationSign = Math.sign(deltaRotationAngles[0]);
      deltaRotationAngles[0] -= rotationSign;

      worldMatrix2 = utils.multiplyMatrices(
        utils.MakeTranslateMatrix(3, 0, 0),
        getQuaternionMatrix(rotationSign, 0, 0)
      );
    } else if (deltaRotationAngles[1] != 0) {
      let rotationSign = Math.sign(deltaRotationAngles[1]);
      deltaRotationAngles[1] -= rotationSign;

      worldMatrix2 = utils.multiplyMatrices(
        utils.MakeTranslateMatrix(3, 0, 0),
        getQuaternionMatrix(0, rotationSign, 0)
      );
    } else if (deltaRotationAngles[2] != 0) {
      let rotationSign = Math.sign(deltaRotationAngles[2]);
      deltaRotationAngles[2] -= rotationSign;

      worldMatrix2 = utils.multiplyMatrices(
        utils.MakeTranslateMatrix(3, 0, 0),
        getQuaternionMatrix(0, 0, rotationSign)
      );
    }
  } else {
    if (deltaRotationAngles[0] != 0) {
      ...
      worldMatrix = getQuaternionMatrix(rotationSign, 0, 0);
    } else if (deltaRotationAngles[1] != 0) {
      let rotationSign = Math.sign(deltaRotationAngles[1]);
      deltaRotationAngles[1] -= rotationSign;

      worldMatrix = getQuaternionMatrix(0, rotationSign, 0);
    } else if (deltaRotationAngles[2] != 0) {
      let rotationSign = Math.sign(deltaRotationAngles[2]);
      deltaRotationAngles[2] -= rotationSign;

      worldMatrix = getQuaternionMatrix(0, 0, rotationSign);
    }
  }
  ...
  /*
  i++;
  let worldMatrix2 = utils.multiplyMatrices(
    projectionMatrix,
    utils.MakeRotateYMatrix(i)
  );
  gl.uniformMatrix4fv(
    program.WmatrixUniform,
    gl.FALSE,
  ...
  gl.drawElements(
    gl.TRIANGLES,
    startIndex[slider + 2] - startIndex[slider + 1],
    gl.UNSIGNED_SHORT,
    startIndex[slider + 1] * 2
  );*/
