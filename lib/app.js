...
var projectionMatrix, perspProjectionMatrix, viewMatrix, worldMatrix;
...
var obj_clicked_pos_near = [0.0, 0.0, 0.0];
var obj_clicked_pos_far = [0.0, 0.0, 0.0];
//final 3d coordinates of the point to be passed
var obj_pos = [];
//final 3d coordinates of the point to be passed (secondo metodo con altra intersezione)
var obj_pos2 = [];
...
var lastMouseX = -100,
  ...
  function doMouseDown(event) {
    ...
    starting_MouseX = event.pageX;
  
    starting_MouseY = event.pageY;
    starting_MouseX -= canvas.offsetLeft;
    starting_MouseY -= canvas.offsetTop;
    ...
    starting_MouseY = canvas.height - starting_MouseY;
  
    console.log("start: x " + starting_MouseX + " , y " + starting_MouseY);
  
    //USING unPROJECT FUNCTION TO GET COORDINATE OF THE NEAREST POINT
  
    // ------------------ //
    // da recuperare in un altro modo sicuro 
    var viewport = [0.0, 0.0, canvas.width, canvas.height];
  
    GLU.unProject(
      starting_MouseX, starting_MouseY, -1.0, viewMatrix, projectionMatrix, viewport, obj_clicked_pos_near
    );
    GLU.unProject(
      starting_MouseX, starting_MouseY, 1.0, viewMatrix, projectionMatrix, viewport, obj_clicked_pos_far
    );
  
    // +++++++++ controllo ++++++++++++ //
    console.log(
      "near: " + obj_clicked_pos_near[0] + " , " + obj_clicked_pos_near[1] + " , " + obj_clicked_pos_near[2]
    );
    console.log(
      "far: " + obj_clicked_pos_far[0] + " , " + obj_clicked_pos_far[1] + " , " + obj_clicked_pos_far[2]
    );
   //valori di prova per la funzione di intersezione con un piano dando una normale e un punto
    var testNormal = [1.0, 0.0, 0.0];
    var testPoint = [1.0, 1.0, 1.0];
  
    obj_pos = linePlaneIntersection (obj_clicked_pos_far - obj_clicked_pos_near, obj_clicked_pos_near, testNormal, testPoint );
    console.log ("pos: " + obj_pos[0] + " , " + obj_pos[1] + " , " + obj_pos[2]);
    
    //valori di prova per la funzione di intersezione con un piano dando tre punti sul piano
    var p1 = [1.0,0.0,0.0];
    var p2 = [0.0,0.0,0.0];
    var p3 = [0.0,1.0,0.0];

    obj_pos2 =  linePlaneIntersection2(p1,p2,p3,obj_clicked_pos_near, obj_clicked_pos_far);
    console.log ("pos: " + obj_pos2[0] + " , " + obj_pos2[1] + " , " + obj_pos2[2]);
...
function doMouseUp(event) {
  e = lastMouseX;
  ending_MouseY = lastMouseY;
  lastMouseX = -100;
  lastMouseY = -100;
  leftMousePressed = false;
  console.log("finish: " + e + " , " + ending_MouseY);

  directionOfMovement();
}

function directionOfMovement() {
  deltaX = e - starting_MouseX;
  deltaY = ending_MouseY - starting_MouseY;

  console.log("delta: " + deltaX + " , " + deltaY);

  direction_Of_Movement = [deltaX, deltaY, 1];
}
...
function doMouseMove(event) {
  if (leftMousePressed) {
    ...
    // console.log(
    //   "started mouse drag event x,y = " + lastMouseX + " " +lastMouseY + "  " + event.which
    // );
  }
}
...
function drawScene() {
  ...
  gl.drawElements(
    gl.TRIANGLES,
    startIndex[slider + 1] - startIndex[slider],
    gl.UNSIGNED_SHORT,
    startIndex[slider] * 2
  );
  ...
  gl.drawElements(
    gl.TRIANGLES,
    startIndex[totMesh + 1] - startIndex[totMesh],
    gl.UNSIGNED_SHORT,
    startIndex[totMesh] * 2
  );
