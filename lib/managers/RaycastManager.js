function RaycastManager() {
  ...
  this.getIntersectedSide = function (rayOrigin, r2) {
    ...
    //now let's find t
    //up side
    t = (sideHalfLength - rayOrigin[1]) / y_coeff;
    x = t * x_coeff + rayOrigin[0];
    y = sideHalfLength;
    z = t * z_coeff + rayOrigin[2];
    if (isInsideBound(x, sideHalfLength) && isInsideBound(z, sideHalfLength)) {
    ...
    //right side
    t = (sideHalfLength - rayOrigin[0]) / x_coeff;
    x = sideHalfLength;
    y = t * y_coeff + rayOrigin[1];
    z = t * z_coeff + rayOrigin[2];
    if (isInsideBound(y, sideHalfLength) && isInsideBound(z, sideHalfLength)) {
      distance = getPointToPointDistance([x, y, z], rayOrigin);
      if (distance < closestDistance) {
        closestDistance = distance;
    ...
    //front side
    t = (sideHalfLength - rayOrigin[2]) / z_coeff;
    x = t * x_coeff + rayOrigin[0];
    y = t * y_coeff + rayOrigin[1];
    z = sideHalfLength;
    if (isInsideBound(x, sideHalfLength) && isInsideBound(y, sideHalfLength)) {
      distance = getPointToPointDistance([x, y, z], rayOrigin);
      if (distance < closestDistance) {
        closestDistance = distance;
