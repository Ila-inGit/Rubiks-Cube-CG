function MouseEventsManager() {
  ...
  this.doMouseDown = function (event) {
    ...
    raycastManager.firstRaycastResult = raycastManager.doRaycast(event);
  ...
  this.doMouseUp = function (event) {
    console.log(
      "___________________cube: " + rubiksCube.cubes[2][2][2].coordinates
    );
    this.lastMouseX = -100;
    this.lastMouseY = -100;
    this.isMovingCamera = false;
    ...
    if (!isRotating && raycastManager.hasHit(firstRaycastResult)) {
      ...
      console.log("axisToRotate: " + rotationAxis);
      console.log(
        "cubeToRotateIndexes:  " +
          "x: " +
  ...
  this.doMouseWheel = function (event) {
    rubiksCube.cubes[2][2][2].translationMatrix = utils.MakeTranslateMatrix(
      0,
      0,
      100
    );
